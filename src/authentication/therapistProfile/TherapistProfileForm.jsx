...
import { FaUserCircle, FaGraduationCap, FaStar, FaCheckCircle } from 'react-icons/fa';
...
const getValidationSchema = (step) => {
  switch (step) {
    ...
    case 3:
      return Yup.object().shape({
        specializations: Yup.array().min(1, 'At least one specialization is required').required('Specializations is required'),
        languages: Yup.array().min(1, 'At least one language is required').required('Languages is required'),
      });
    default:
      return Yup.object().shape({});
  }
...
const TherapistProfileForm = () => {
  ...
  const renderStep = (step, formikProps) => {
    switch (step) {
      ...
      case 2:
        return <TherapistStep2EducationCertification {...formikProps} isEducationModalOpen={isEducationModalOpen} setIsEducationModalOpen={setIsEducationModalOpen} isCertificationModalOpen={isCertificationModalOpen} setIsCertificationModalOpen={setIsCertificationModalOpen} isExperienceModalOpen={isExperienceModalOpen} setIsExperienceModalOpen={setIsExperienceModalOpen} />;
      case 3:
    ...
    >
      {(formikProps) => (
        <Layout img={signUp_brain}>
          <div className={isEducationModalOpen || isCertificationModalOpen || isExperienceModalOpen ? "opacity-10 pointer-events-none w-full max-w-lg rounded-lg font-sans text-[18px] leading-[100%] tracking-[0px] mt-10 pt-10" : "w-full max-w-lg rounded-lg font-sans text-[18px] leading-[100%] tracking-[0px] mt-10 pt-6 h-screen"}>
            <div className="mb-6">
                  ...
                  <div key={step.id} className="relative flex-1 flex flex-col items-center">
                    ...
                    {index !== steps.length - 1 && (
                      <div className="absolute top-1/3 left-[calc(50%+25px)] w-[calc(100%-50px)] -translate-y-1/2">
                        ...
                        <div
                          className={`h-[2px] ${
                            currentStep > step.id ? 'bg-[#65C6F2]' : 'bg-gray-300'
                          }`}
                        ></div>
                      </div>
                    )}
